...
typedef enum DmacIdDef {
    DMAC_ID_AHB, /**< DMAC on the AHB bus */
    DMAC_ID_AXI, /**< DMAC on the AXI bus */
    DMAC_ID_MAX, /**< DMAC max ID */
} DmacIdDef;
...
typedef struct DmacCtlReg {
     ...
     * Generally, we assign separately src_xfer_width and dst_xfer_width with
     ...
     * transfer length.
     * it's meaning that Number of data items, each of width src_xfer_width
     * or dst_xfer_width (NOT BYTE NUMBER), to be read/write from/to the
    ...
    /*
     * Source gather enable:Scatter on the destination side
     * is applicable only when the CTLx.DINC bit indicates an
     * incrementing or decrementing address control.
     */
    uint32_t src_gth_en : 1;

    /*
     * Destination scatter enable:Gather on the source side is
     * applicable only when the CTLx.SINC bit indicates an
     * incrementing or decrementing address control.
     */
    uint32_t dst_sct_en : 1;

    /*
     * Transfer type and flow controller for this transfer.
     * value @see DmacTransDirAndFc
     */
    uint32_t tt_fc : 3;

    /*
     * Destination/Source Master Select. Identifies the Master Interface
     * layer where the destination device (peripheral or memory) resides.
     * 00 = AHB master 1; 01 = AHB master 2; 10 = AHB master 3;
     * 11 = AHB master 4.
     */
    uint32_t dms : 2;
    uint32_t sms : 2;
...
typedef struct DmacXferCfg {
    ...
    /**
     * How many bytes you want to transfer,be used to calculate block
     * transfer size
     */
    uint32_t len;
     ...
     * Note: corresponding to SGRx Register
     */
     ...
     * Note: corresponding to DSRx Register
     */
    ...
    /**
     * The trigger level of peripheral FIFO,src_msize and dst_msize be
     * set according to it
     */
...
typedef struct DmacDevice {
    ...
    const DmacHwConfig *hw_cfg; /**< DMAC hardware
                                  configuration */
    DmacChnMap *ch_map; /**< The channel status info map */
    DmacXferCfg *xfer_cfg; /**< The channel configuration */
    /** Indicates the maximum number of read/write
     * operations from the source or destination for one
     * block. Each read/write operation is (1 << xfer_width)
     * bytes
     */
    uint32_t max_blk_ts;
    /** Maximum value of burst transaction size that can be
     * programmed
     */
    uint32_t max_msize;
...
typedef struct DmaCbAndParam {
    DmaXferCallback callback; /**< The callback for DMA transfer done for normal
                                 mode; Or the callback for DMA block transfer
                                 done for cyclic mode */
    const void *param; /**< The parameter required in the callback which can be
                          instances of peripherals device or other types */
} DmaCbAndParam;
...
typedef struct DmacOperation {
    int (*get_resource)(const DmacDevice *device, DmacDevType src,
                        DmacDevType dst);
    int (*init)(const DmacDevice *device, DmacXferCfg *xfer_cfg);
    int (*start)(const DmacDevice *device, DmacXferCfg *xfer_cfg,
                 DmaCbAndParam *xfer_cb);
    int (*stop)(const DmacDevice *device, DmacXferCfg *xfer_cfg);
...
/**
 * @brief Allocate a available dmac channel
 * @param[in] device dmac device
 * @param[in] src source Peripheral
 * @param[in] dst destination Peripheral
 * @param[in] fifo_width peripheral fifo width
 * @param[in] mux_id peripheral mux id, designed by soc,if direction is M2M,
 * please use macro DMAC_INVALID_MUX_ID
 * @param[in] block_ts the current transmission read/write times of each block
 * which cannot exceed max_blk_ts
 * @return  DmacXferCfg pointer: on success,  NULL : error
 */
...
/**
 * @brief Init DMAC channel_x registers params
 * @param[in] device the Dmac device
