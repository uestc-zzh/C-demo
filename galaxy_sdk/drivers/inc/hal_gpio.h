 ...
 *
 * @file hal_gpio.h
 * @brief public head file of GPIO HAL
...
typedef struct GpioDevice {
    ...
    const void *bsp_ops; /**< Gpio bsp extent operations */
...
typedef struct GpioOperations {
    int (*io_dir)(const GpioDevice *device, unsigned char io_idx, uint8_t b_out,
                  IoPullCtrl pull);
    uint8_t (*get_dir)(const GpioDevice *device, unsigned char io_idx);
    int (*io_irq)(const GpioDevice *device, unsigned char io_idx,
                  uint8_t b_irq);
    int (*irq_set)(const GpioDevice *device, unsigned char io_idx, uint8_t b_st,
                   GpioIrqTrigger trig);
    int (*output_set)(const GpioDevice *device, unsigned char io_idx,
                      uint8_t level);
    ...
    int (*irq_enable)(const GpioDevice *device, unsigned char io_idx,
                      uint8_t reolad, void *handler);
